...
import 'package:authentication_repository/data_source/authentication_data_source.dart';
...
class AuthenticationRepository {
  ...
  AuthenticationRepository({required AuthenticationDataSource dataSource}) : _dataSource = dataSource;
  ...
  Stream<AuthenticationStatus> get status async* {
    final token = await SecureStorageHelper.readValueByKey(SecureStorageKey.jwt);
    if (token == null) {
  ...
  Future<void> logout() async {
    try {
      return SecureStorageHelper.deleteAllKeys().then((_) => _controller.add(AuthenticationStatus.unauthenticated));
    } catch (e, stackTrace) {
